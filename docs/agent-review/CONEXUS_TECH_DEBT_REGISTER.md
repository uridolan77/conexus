...
| TD-005 | Streaming accounting | `backend/app/services/gateway_service.py` streaming wrapper | P1 | Stream completion logging depends on finishing stream and receiving usage; interruptions yield partial token/cost fields. | BO usage/cost reporting may be skewed during stream failures. | Add tests for stream interruption; log clearer failure codes; consider best-effort usage capture. | **partial** — stream-no-usage test added; interrupt/timeout tests already existed (2nd pass) |
